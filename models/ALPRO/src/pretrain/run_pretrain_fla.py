...
from src.configs.config_fla import shared_configs
...
from src.modeling.fla_models import FlaForPretrain
...
def setup_model(cfg, device=None):
    ...
    text_enc_cfg = load_json(cfg.text_enc_config)[cfg.text_enc_model]
    # text_enc_cfg = BertConfig(**text_enc_cfg)
    # add model-specific config
    add_attr_list = [
        "max_n_example_per_group",
    ]
    for k in add_attr_list:
        setattr(text_enc_cfg, k, cfg[k])
    LOGGER.info(f"text encoder config {pprint.pformat(text_enc_cfg.to_dict())}")

    LOGGER.info("setup e2e model")

    if cfg.model_type == 'pretrain':
        # initialize cnn config
        visual_enc_cfg = load_json(cfg.visual_enc_cfg)[cfg.visual_enc_model]
        visual_enc_cfg['img_size'] = cfg.crop_img_size

        text_dec_cfg = load_json(cfg.text_dec_cfg)[cfg.visual_dec_model]

        model = FlaForPretrain(
            cfg,
            text_enc_cfg,
            visual_enc_cfg,
            text_dec_cfg,
        ...
        if cfg.e2e_weights_path:
            ...
            num_patches = (cfg.crop_img_size // visual_enc_cfg['patch_size']) ** 2
