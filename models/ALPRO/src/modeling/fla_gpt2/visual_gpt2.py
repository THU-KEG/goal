from src.modeling.fla_gpt2.configuration_gpt2 import GPT2Config
from src.modeling.fla_gpt2.modeling_gpt2 import GPT2LMHeadModel
from src.modeling.fla_gpt2.tokenization_gpt2 import GPT2Tokenizer
from src.modeling.fla_gpt2.utils_gpt2 import *
...
class VisualGPT2(nn.Module):
    ...
    def forward(self, memory_embed, memory_mask, target_input_ids, target_input_mask, pos_embed=None, past_key_values=None):
    ...
    def generate(self, memory_embed, memory_mask, pos_embed=None, target_input_ids=None, target_input_mask=None):
        ...
        while not all(bsz_ends) and i < self.max_dec_length-1:
            logits, loss, hs = self.forward(memory_embed, memory_mask, caption_ids, caption_mask, pos_embed)
            logits = logits[:, i, :]
    ...
    # def generate(self, memory_embed, memory_mask, pos_embed=None, target_input_ids=None, target_input_mask=None, context_tokens=None):
    #     """ Generate the target sequence given encoder memory.
    #           Calculate the loss if the target_ids and target_mask are given.
    #       Args:
    #         target_input_ids [B, max_dec_len]: the ids of expected target captions for the computation of loss.
    #         context_tokens [B, ...]: a list of token lists.
    #       Return:
    #         caption_ids [B, max_dec_len]: the targets padded with start/end tokens.
    #         caption_logits [B, max_dec_len]: the targets padded with end token logits.
    #     """
    ...
    #         caption_ids, caption_mask = self.prepare_input(self.tokenizer, self.max_dec_length, batch_size=bsz, device=memory_embed.device)
    #         past_key_values = None
    #     else:
    #         caption_ids, caption_mask = self.prepare_input(self.tokenizer, self.max_dec_length, context_tokens, device=memory_embed.device)
    #         logits, loss, past_key_values = self.forward(memory_embed, memory_mask, caption_ids, caption_mask, pos_embed)
    #         pred_ids = torch.argmax(logits, axis=-1) # (B, S)
    #         baz_tokens = []
    #         for i in range(len(pred_ids)):
    #             ctx_len = len(context_tokens[i])
    #             baz_tokens.append([self.tokenizer.convert_ids_to_tokens([pred_ids[ctx_len]])]) # (B, 1)
    #         caption_ids, caption_mask = self.prepare_input(self.tokenizer, self.max_dec_length, baz_tokens, device=memory_embed.device)
    ...
    #     i = 0
    #     while not all(bsz_ends) and i < self.max_dec_length-1:
    #         logits, loss, past_key_values = self.forward(memory_embed, memory_mask, caption_ids, caption_mask, pos_embed, past_key_values)
    #         # logits = logits[:, i, :]
    #         logits = logits[:, 1, :]
    ...
    @classmethod
    def prepare_input(self, tokenizer, max_dec_length, batch_tokens=None, batch_size=None, device=None) -> Tuple:
        ...
        if device is not None:
            ...
            target_caption_mask = target_caption_mask.to(device)
