...
class VideoTransformer(nn.Module):
    ...
    def forward(self, memory_embed, memory_mask, target_input_ids, target_input_mask, pos_embed=None):
        ...
        memory_embed = memory_embed.permute(1, 0, 2)
        memory_mask = memory_mask <= 0
        if pos_embed is not None:
            pos_embed = pos_embed.permute(1, 0, 2)
            
        # target = self.embeddings(target_input_ids[:, :-1]).permute(1, 0, 2)
        target = self.embeddings(target_input_ids).permute(1, 0, 2)
        ...
        query_pos = query_pos.expand(query_pos.shape[0], bsz, query_pos.shape[2])
        
        # hs = self.decoder(target, memory_embed, memory_key_padding_mask=memory_mask, tgt_key_padding_mask=target_input_mask[:, :-1],
        hs = self.decoder(target, memory_embed, memory_key_padding_mask=memory_mask, tgt_key_padding_mask=target_input_mask,
